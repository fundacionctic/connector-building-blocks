...
@app.after_startup
async def send_mock_message_on_startup():
    await asyncio.sleep(random.random() * 2)

    await broker.publish(
        queue="aggregation-task-queue",
        exchange=exchange,
        message=ConsumptionAggregationTaskMessage(
            location="Bimenes",
            sink_url="http://mock.datacellar.eu:8000/consumption/results",
        ),
    )
